...
loop(MapOfEmotions) ->
  receive
    {Params, CallerPid} ->
      {FilteredText, EngagementRatio, IsRedacted} = Params,
      ...
      CallerPid
      ! {sentiment_calculated, {FilteredText, EngagementRatio, IsRedacted, SentimentScore}},
      loop(MapOfEmotions)
  end.
...
start() ->
  MapOfEmotions =
    #{"abandon" => -2,
      ...
      "zealots" => -2,
      "zealous" => 2},
  Pid = spawn_link(fun() -> loop(MapOfEmotions) end),
  register(sentiment, Pid),
  {ok, Pid}.

get_score([], _MapOfEmotions) ->
  0;
get_score(Word, MapOfEmotions) ->
  FoundValue = maps:find(Word, MapOfEmotions),
